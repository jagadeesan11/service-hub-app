...
-- Verification only: promotes one probe account so the owner app's writes can
-- be tested from a real admin session. The service key cannot do this —
-- prevent_self_role_escalation refuses it, which is correct.
