-- create_booking learns about promo codes.
--
-- Dropped and recreated rather than replaced: adding a parameter would create
-- an overload, and two functions differing only by a defaulted argument make
...
revoke all on function public.create_booking(
  uuid, timestamptz, uuid, uuid[], text, text, text, text, text, boolean, text, text
) from public;
...
comment on function public.create_booking is
  'The only booking-creation path open to customers. Prices the job and re-validates any promo code server-side; the caller cannot set total_price, user_id or the discount.';
